...
const BADGE_STATS_KEY = "familyplanner-badge-stats-v1";
const LEVELS = [
...
function getStoredLastBadgeMap() {
  return readJsonStorage(LAST_BADGE_KEY, {});
}

function setStoredLastBadge(childId, badgeId) {
  const current = getStoredLastBadgeMap();
  const next = { ...current, [childId]: badgeId };
  return writeJsonStorage(LAST_BADGE_KEY, next);
}

function getStoredBadgeStatsMap() {
  return readJsonStorage(BADGE_STATS_KEY, {});
}

function ensureStoredBadgeEntries(childId, badgeIds = []) {
  const current = getStoredBadgeStatsMap();
  const childStats = { ...(current?.[childId] || {}) };
  let changed = false;
  badgeIds.filter(Boolean).forEach((badgeId) => {
    if (!childStats[badgeId] || typeof childStats[badgeId] !== "object") {
      childStats[badgeId] = { count: 1, lastEarnedAt: getTodayISO() };
      changed = true;
      return;
    }
    if (!Number.isFinite(Number(childStats[badgeId].count)) || Number(childStats[badgeId].count) < 1) {
      childStats[badgeId] = { ...childStats[badgeId], count: 1, lastEarnedAt: childStats[badgeId].lastEarnedAt || getTodayISO() };
      changed = true;
    }
  });
  if (!changed) return current;
  const next = { ...current, [childId]: childStats };
  return writeJsonStorage(BADGE_STATS_KEY, next);
}

function incrementStoredBadgeCount(childId, badgeId) {
  const current = getStoredBadgeStatsMap();
  const childStats = { ...(current?.[childId] || {}) };
  const prev = childStats[badgeId] || { count: 0 };
  childStats[badgeId] = {
    ...prev,
    count: Math.max(0, Number(prev.count || 0)) + 1,
    lastEarnedAt: getTodayISO(),
  };
  const next = { ...current, [childId]: childStats };
  writeJsonStorage(BADGE_STATS_KEY, next);
  return next;
}
...
function countApprovedDayRuns(tasks = [], minDays = 1) {
  const safeMin = Math.max(1, Number(minDays || 1));
  const uniqueDays = Array.from(new Set(tasks.map(getTaskAnchorDate).filter(Boolean))).sort();
  ...
  for (let i = 1; i < uniqueDays.length; i += 1) {
    ...
    if (diff === 1) runLength += 1;
    else {
      count += Math.floor(runLength / safeMin);
      runLength = 1;
    }
  }
  count += Math.floor(runLength / safeMin);
...
function getChildBadgeState({ child, data, lifetimeCoins = 0, childBirthDatesMap = {}, childCountdownsMap = {}, referenceDate = getTodayISO(), todayActiveTasks = null, todayDoneCount = null }) {
  ...
  const streak3Count = countApprovedDayRuns(approvedTasks, 3);
  const streak7Count = countApprovedDayRuns(approvedTasks, 7);
  const badges = BADGE_DEFS.map((badge) => {
    ...
    let repeatCount = earned ? 1 : 0;
    if (badge.id === "poetskampioen") repeatCount = Math.floor(rawValue / 10);
    if (badge.id === "zelfdoener") repeatCount = Math.floor(rawValue / 10);
    if (badge.id === "opruimpro") repeatCount = Math.floor(rawValue / 10);
    if (badge.id === "boekenwurm") repeatCount = Math.floor(rawValue / 5);
    if (badge.id === "doorzetter") repeatCount = streak3Count;
    if (badge.id === "weekheld") repeatCount = streak7Count;
    if (badge.id === "coin-starter") repeatCount = Math.floor(lifetimeCoins / 50);
    if (badge.id === "level-5-held") repeatCount = level >= 5 ? 1 + Math.floor((level - 5) / 5) : 0;
    if (badge.id === "beloningsbaas") repeatCount = approvedRewardCount;
    if (badge.id === "bijna-jarig") repeatCount = earned ? 1 : 0;
    if (badge.id === "alles-klaar") repeatCount = allDoneToday ? 1 : 0;
    return { ...badge, value: rawValue, earned, progress, progressText, repeatCount };
  });
  return {
...
function BadgeShowcase({ badges = [], theme = DEFAULT_THEME, compact = false, title = "Badgekast", subtitle = "", badgeCounts = {} }) {
  return (
    <div style={{ background: "#fff", border: `2px solid ${theme.pri}26`, borderRadius: 22, padding: compact ? 14 : 16, boxShadow: `0 8px 24px ${theme.pri}12` }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns: compact ? 'repeat(auto-fit, minmax(120px, 1fr))' : 'repeat(auto-fit, minmax(150px, 1fr))', gap: compact ? 10 : 12 }}>
        {badges.map((badge) => {
          const earnedCount = Math.max(0, Number(badge.count ?? badgeCounts?.[badge.id]?.count ?? 0));
          return (
          <div key={badge.id} style={{
            ...
            <div style={{ marginTop: 8, display:'flex', justifyContent:'center' }}>
              <div style={{ padding:'5px 9px', borderRadius:999, fontSize:11, fontWeight:900, letterSpacing:'.01em', background: badge.earned ? `${theme.pri}14` : 'rgba(148,163,184,0.12)', color: badge.earned ? theme.priD : '#64748b', border: badge.earned ? `1px solid ${theme.pri}2f` : '1px solid rgba(148,163,184,0.16)' }}>
                {earnedCount > 0 ? `Behaald: ${earnedCount}x` : 'Nog niet behaald'}
              </div>
            </div>
...
function ChildView({ data, db, activeKid, kidTab, setKidTab, playTaskDone, playAllDone, playSpend, onAllDone, coinTargetRef, getLifetimeCoinsForChild, childBirthDatesMap, childCountdownsMap, childActivitiesMap }) {
  ...
  const [badgeStatsMap, setBadgeStatsMap] = useState(() => getStoredBadgeStatsMap());
  ...
  useEffect(() => {
    ...
    setBadgeStatsMap(getStoredBadgeStatsMap());
  }, [activeKid]);
  ...
  const childBadgeStats = badgeStatsMap?.[activeKid] || {};
  const mergedBadges = badgeState.badges.map((badge) => ({
    ...badge,
    count: Math.max(
      0,
      Number(childBadgeStats?.[badge.id]?.count || 0),
      Number(badge.repeatCount || 0)
    ),
  }));
  ...
  useEffect(() => {
    if (!activeKid) return;
    const ensured = ensureStoredBadgeEntries(activeKid, earnedBadges.map(b => b.id));
    setBadgeStatsMap(ensured);
  }, [activeKid, earnedBadgeIdsKey]);
  ...
  useEffect(() => {
    ...
    if (prev.length > 0) {
      ...
      if (newlyUnlocked.length) {
        ...
        let nextStats = getStoredBadgeStatsMap();
        newlyUnlocked.forEach((badge) => {
          nextStats = incrementStoredBadgeCount(activeKid, badge.id);
        });
        setBadgeStatsMap(nextStats);
      ...
      {activeBadgeUnlock && (
        <div style={{ position:'fixed', inset:0, zIndex:9998, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none' }}>
          ...
          <div style={{ position:'relative', zIndex:2, width:'min(92vw, 420px)', borderRadius:32, padding:'24px 24px 22px', background:'linear-gradient(135deg, rgba(255,255,255,.98), rgba(255,247,255,.96))', boxShadow:`0 28px 90px ${activeBadgeUnlock.glow}, 0 0 0 3px ${th.pri}22`, border:`2px solid ${th.pri}30`, textAlign:'center', animation:'feestCardIn .42s cubic-bezier(.34,1.56,.64,1) both', pointerEvents:'auto' }}>
            ...
            <div style={{ marginTop:14, display:'inline-flex', alignItems:'center', gap:8, padding:'9px 14px', borderRadius:999, background:`${th.pri}14`, color:th.priD, fontSize:13, fontWeight:900, border:`1px solid ${th.pri}2f` }}>Totaal gehaald: {Math.max(1, Number((badgeStatsMap?.[activeKid] || {})?.[activeBadgeUnlock.id]?.count || activeBadgeUnlock.count || 1))}x</div>
            ...
            {todayActivities.length > 0 ? (
              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                {todayActivities.map(item => (
                  <div key={item.id} style={{ background:`${th.pri}14`, color:th.priD, border:`1px solid ${th.pri}2e`, borderRadius:999, padding:'14px 18px', fontSize:14, fontWeight:800, display:'flex', alignItems:'center', gap:12 }}>
                    <span style={{ fontSize: 60, lineHeight: 1 }}>{item.emoji || guessActivityEmoji(item.title)}</span>
...
function PurchasesTab({ data, db, getChild }) {
  ...
  const getHandledDate = (r) => fmtDate(r.approvedOn || r.approved_at || r.handledOn || r.handled_at || r.updated_at || r.updatedAt || (r.status !== "pending" ? r.date : ""));

  const statusLabel = (r) => {
    if (isPenaltyRedemption(r)) return { text: "⚠️ Straf uitgevoerd", bg: "rgba(245, 158, 11, .16)", color: "#fde68a" };
    if (r.status === "approved") return { text: "✅ Goedgekeurd", bg: "rgba(34, 197, 94, .16)", color: "#bbf7d0" };
    if (r.status === "rejected") return { text: "❌ Afgewezen", bg: "rgba(248, 113, 113, .16)", color: "#fecaca" };
    return { text: "⏳ Wacht op goedkeuring", bg: "rgba(250, 204, 21, .16)", color: "#fde68a" };
  };

  const infoLine = (label, value) => (
    <div style={{ fontSize: 12, color: '#d8e3f4', lineHeight: 1.45 }}><span style={{ color: '#aebdd5', fontWeight: 800 }}>{label}:</span> {value}</div>
  );
  ...
  const RedemptionCard = ({ r }) => {
    ...
    const coinsText = penalty ? `${Math.abs(r.cost)}` : `${r.cost}`;
    const title = penalty ? (getPenaltyReason(r) || 'Straf uitgevoerd') : (r.rewardTitle || 'Beloning');
    const sub = penalty ? 'Ecoins afgepakt' : (r.rewardDesc || 'Aankoop of beloningsaanvraag');
    const badge = statusLabel(r);

    return (
      <div className="card" style={parentQuietUi.card({ textAlign: 'center', display:'flex', flexDirection:'column', minHeight: 250 })}>
        <div style={{ fontSize: 42, marginBottom: 8 }}>{r.rewardEmoji || (penalty ? '⚠️' : '🎁')}</div>
        <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 4, color: '#f5f8ff' }}>{title}</div>
        <div style={{ fontSize: 12, color: '#c7d3e7', marginBottom: 8 }}>{sub}</div>
        <div style={{ fontSize: 11, color: 'var(--pri)', fontWeight: 800, marginBottom: 10 }}>{ch ? `${ch.avatar} ${ch.name}` : 'Kind onbekend'}</div>

        <div style={{ alignSelf:'center', background: badge.bg, color: badge.color, borderRadius: 999, padding:'5px 10px', fontSize:11, fontWeight:800, marginBottom: 12 }}>{badge.text}</div>

        <div style={{ display:'grid', gap:6, marginBottom: 12 }}>
          {infoLine('Aangevraagd', requestedDate)}
          {infoLine(penalty ? 'Uitgevoerd' : 'Goedgekeurd', r.status === 'pending' && !penalty ? 'Nog open' : handledDate)}
          {infoLine('Coins', `🪙 ${coinsText}`)}
        </div>

        <div style={{ fontSize: 24, fontWeight: 900, color: penalty ? '#fdba74' : 'var(--yel)', marginTop:'auto', marginBottom: 12 }}>🪙 {coinsText}</div>

        {!penalty && r.status === 'pending' ? (
          <div style={{ display:'grid', gap:8 }}>
            <button className="btn bg bsm" style={{ justifyContent:'center' }} onClick={() => db.approveRedemption(r.id)}>✅ Goedkeuren</button>
            <button className="btn bsm" style={{ justifyContent:'center', background:'var(--red-l)', color:'var(--red)', border:'none' }} onClick={() => db.rejectRedemption(r.id)}>❌ Afwijzen</button>
          </div>
        ) : (
          <div style={{ height: 36 }} />
        )}
    ...
    <div>
      ...
      {pendingList.length > 0 && (
        <div style={{ marginBottom:20 }}>
          <div style={{ fontFamily:"'Baloo 2',cursive", fontSize:15, fontWeight:800, color:'#eef2ff', marginBottom:10 }}>⏳ Wacht op jouw goedkeuring</div>
          <div className="ga">{pendingList.map(r => <RedemptionCard key={r.id} r={r} />)}</div>
        </div>
      )}

      {restList.length > 0 && (
        <div>
          <div style={{ fontFamily:"'Baloo 2',cursive", fontSize:15, fontWeight:800, color:'#eef2ff', marginBottom:10 }}>Eerder behandeld</div>
          <div className="ga">{restList.map(r => <RedemptionCard key={r.id} r={r} />)}</div>
        </div>
      )}
